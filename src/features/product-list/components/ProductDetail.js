...
import { fetchAProductByIdAsync, selectProductById } from '../productSlice'
...
export default function ProductDetail() {
  ...
  const user = useSelector(loggedInUser).id;
  console.log(user)
  const dispatch = useDispatch();
  useEffect(() => {
    // Dispatch the async action when the component mounts
    dispatch(fetchAProductByIdAsync(params));
  }, [dispatch,params]);
  ...
  const handleSubmit = (e) =>{
    e.preventDefault();
    const newItem = {...product,"id":user,"quantity":1};
    delete newItem["id"];
